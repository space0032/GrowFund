...
public class AnalyticsService {
    ...
    public AnalyticsDTO getFarmAnalytics(Long farmId) {
        ...
        List<Investment> investments = investmentRepository.findAll().stream() // Ideally filter by user/farm if linked
                .filter(i -> i.getUser().getId().equals(farm.getUser().getId())) // Assuming link via User
        ...
        for (Investment inv : investments) {
            Long val = inv.getCurrentValue() != null ? inv.getCurrentValue() : inv.getPrincipalAmount();
        ...
        Long totalCropValue = activeCrops.stream()
                .mapToLong(c -> c.getInvestmentAmount()) // Value at cost for now
                .sum();

        // 4. Equipment Value
        List<FarmEquipment> equipment = farmEquipmentRepository.findByFarmId(farmId);
        Long totalEquipmentValue = equipment.stream()
                .mapToLong(e -> e.getEquipment().getCost()) // Value at cost
                .sum();

        // 5. ROI Calculations
        // Historical ROI: (Total Profit from Harvested Crops + Matured Investments
        // Profit) / Total Cost
        ...
        for (Crop c : harvestedCrops) {
            totalHistoricalCost += c.getInvestmentAmount();
        ...
        if (totalHistoricalCost > 0) {
            historicalRoi = ((double) (totalHistoricalRevenue - totalHistoricalCost) / totalHistoricalCost) * 100;
        }

        // Projected ROI for Active Crops (Simple estimation: Cost * 1.5 as baseline)
        // This is a placeholder. Real calculation would need market prices etc.
        double projectedRoi = 15.0; // Flat 15% placeholder for now

        return AnalyticsDTO.builder()
                .totalPortfolioValue(savings + totalInvestmentValue + totalCropValue + totalEquipmentValue)
                .savings(savings)
